---
description: Developer skills for working on the Tavern backend and the react frontend.
---
...
4. **Optionally** update the `models:` section of `tavern/internal/graphql/gqlgen.yml` to bind any GraphQL enum types to their respective `entgo` generated types (e.g. `github.com/spellshift/realm/tavern/internal/ent/<NAME>.<ENUM_FIELD>`)
5. Run `go generate ./tavern/...` from the project root
    - This will trigger generation code in `tavern/internal/ent/generate.go`
6. If you added an annotation for a root query field (see above), you will notice the auto-generated `query.resolvers.go` file has been updated with new methods to query your model (e.g. `func (r *queryResolver) <NAME>s ...`)
    - This must be implemented (e.g. `return r.client.<NAME>.Query().All(ctx)` where NAME is the name of your model)

### Relay Connections

We use Relay connections for pagination (on every query returning multiple results). You can find more information about Relay connections [here](https://relay.dev/graphql/connections.htm). Ent will auto-generate these for us so long as the proper annotations are included, so please add them where applicable!
...
* `tavern/internal/ent/generate.go` is responsible for generating ents defined by the ent schema as well as updating the GraphQL schema and generating related code
* `tavern/internal/ent/generate_code.go` is responsible for generating code for the entgo <-> 99designs/gqlgen GraphQL integration
