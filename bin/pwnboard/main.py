...
if __name__ == "__main__":
    ...
    parser.add_argument('-n', '--name', default='Realm', help="Custom Application Name on PWNBoard")
    parser.add_argument('-i', '--interval', default=5, help="Callback Interval to PWNBoard (seconds). Default to 5 seconds")
    parser.add_argument('-t', '--timediff', default=5, help="Number of seconds from now to check for a beacon response")
    ...
    if auth_session is None:
        print("No auth-session cookie found. Please set it using the environment variable TAVERN_AUTH_SESSION")
        exit(1)
    ...
    while True:
        ...
        graphql_variables = {"input": {
                "lastSeenAtGT": formatted_time
            }
        }

        result = make_graphql_request(args.tavern_url, graphql_query, graphql_variables, cookies)
