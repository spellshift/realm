    ...
    Win32::{
        ...
        System::{
            ...
            SystemServices::{
                ...
                IMAGE_IMPORT_BY_NAME, IMAGE_IMPORT_DESCRIPTOR, IMAGE_ORDINAL_FLAG64,
...
fn virtual_alloc(
    fn_ptr: FnVirtualAlloc,
    err_ptr: FnGetLastError,
    ...
    let buffer_handle: *mut c_void =
        unsafe { fn_ptr(lp_address, dw_size, fl_allocation_type, fl_protect) };
...
fn get_proc_address(
    ...
    let proc_handle = match unsafe { fn_ptr(h_module, lp_proc_name) } {
        ...
        None => {
            let error_code = unsafe { err_ptr() };
            panic!(
                "Failed to find function {:?} in module. Last error returned: {}",
                lp_proc_name, error_code
            );
        }
    };
...
fn load_library_a(
    ...
    if library_handle == 0 {
        ...
        if error_code != 0 {
            panic!(
                "Failed to load library. Last error returned: {}",
                error_code
            );
        }
    }
...
impl PeFileHeaders64 {
    fn new(dll_bytes_ptr: *mut c_void) -> Self {
        ...
        let mut section_headers: [IMAGE_SECTION_HEADER; MAX_PE_SECTIONS] =
            [null_section; MAX_PE_SECTIONS];
        // let mut section_headers: [IMAGE_SECTION_HEADER; MAX_PE_SECTIONS] = unsafe{ core::mem::zeroed() };
        let optional_headers_start_ptr = unsafe {
            &(*(nt_headers_base_ptr as *mut IMAGE_NT_HEADERS64)).OptionalHeader as *const _ as usize
        };
        let section_headers_start_ptr =
            optional_headers_start_ptr + nt_headers.FileHeader.SizeOfOptionalHeader as usize;
        ...
        for section_index in 0..nt_headers.FileHeader.NumberOfSections {
            ...
            cur_section_ptr = (cur_section_ptr as usize
                + core::mem::size_of::<IMAGE_SECTION_HEADER>() as usize)
...
fn relocate_dll_image_sections(
    new_dll_base: *mut c_void,
    old_dll_bytes: *const c_void,
    pe_file_headers: &PeFileHeaders64,
) -> Result<(), &str> {
    for (section_index, section) in pe_file_headers.section_headers.iter().enumerate() {
        if section_index >= pe_file_headers.nt_headers.FileHeader.NumberOfSections as usize {
            return Ok(());
        }
        ...
        unsafe {
            copy_nonoverlapping(
                section_bytes as *const u8,
                section_destination as *mut u8,
...
fn process_dll_image_relocation(
    new_dll_base: *mut c_void,
    pe_file_headers: &PeFileHeaders64,
    image_base_delta: isize,
) -> Result<(), &str> {
    let relocation_directory: IMAGE_DATA_DIRECTORY =
        pe_file_headers.nt_headers.OptionalHeader.DataDirectory
            [IMAGE_DIRECTORY_ENTRY_BASERELOC as usize];
    if relocation_directory.Size == 0 {
        // No relocations to process
        return Ok(());
    }
    let mut relocation_block_ref: *mut IMAGE_BASE_RELOCATION = (new_dll_base as usize
        + relocation_directory.VirtualAddress as usize)
        as *mut IMAGE_BASE_RELOCATION;
    loop {
        let relocation_block = unsafe { *relocation_block_ref as IMAGE_BASE_RELOCATION };
        if relocation_block.SizeOfBlock == 0 || relocation_block.VirtualAddress == 0 {
            return Ok(());
        }
        ...
        let relocation_block_entries_count = (relocation_block.SizeOfBlock as usize
            - core::mem::size_of::<IMAGE_BASE_RELOCATION>() as usize)
            / BaseRelocationEntry::c_size();
        let mut relocation_entry_ptr: *mut u16 = (relocation_block_ref as usize
            + core::mem::size_of::<IMAGE_BASE_RELOCATION>() as usize)
            as *mut u16;
        for _index in 0..relocation_block_entries_count {
            let relocation_entry: BaseRelocationEntry =
                BaseRelocationEntry::new(unsafe { *relocation_entry_ptr });
            if relocation_entry.reloc_type as u32 == IMAGE_REL_BASED_DIR64
                || relocation_entry.reloc_type as u32 == IMAGE_REL_BASED_HIGHLOW
            {
                let addr_to_be_patched = (new_dll_base as usize
                    + relocation_block.VirtualAddress as usize
                    + relocation_entry.offset as usize)
                    as *mut usize;
                let new_value_at_addr = unsafe { *addr_to_be_patched } + image_base_delta as usize;
                unsafe { *addr_to_be_patched = new_value_at_addr };
            }
            relocation_entry_ptr =
                (relocation_entry_ptr as usize + BaseRelocationEntry::c_size()) as *mut u16;
        }
        relocation_block_ref = (relocation_block_ref as usize
...
fn process_import_address_tables(
    new_dll_base: *mut c_void,
    pe_file_headers: &PeFileHeaders64,
    load_library_a_fn: FnLoadLibraryA,
    get_proc_address_fn: FnGetProcAddress,
    get_last_error_fn: FnGetLastError,
) -> Result<(), &str> {
    let import_directory: IMAGE_DATA_DIRECTORY =
        pe_file_headers.nt_headers.OptionalHeader.DataDirectory
    ...
    let mut base_image_import_table: *mut IMAGE_IMPORT_DESCRIPTOR = (new_dll_base as usize
    ...
    loop {
        ...
        let library_name_ptr =
            (new_dll_base as usize + import_table_descriptor.Name as usize) as *const i8;
        let library_name = unsafe { CStr::from_ptr(library_name_ptr) };
        let library_handle = load_library_a(
            load_library_a_fn,
            get_last_error_fn,
            library_name.as_ptr() as *const u8,
        );
        if library_handle != 0 {
            #[cfg(target_arch = "x86_64")]
            let mut library_thunk_ptr = (new_dll_base as usize
                + import_table_descriptor.FirstThunk as usize)
            ...
            loop {
                ...
                if image_snap_by_ordinal(unsafe { library_thunk.u1.Ordinal as usize })? {
                    ...
                    let function_ordinal_ptr =
                        image_ordinal(unsafe { library_thunk.u1.Ordinal as usize })? as *const u8;
                    // Get the address of the function using `GetProcAddress` and update the thunks reference.
                    library_thunk.u1.Function = get_proc_address(
                        get_proc_address_fn,
                        get_last_error_fn,
                        library_handle,
                    ...
                    let tmp_new_func_addr = get_proc_address(
                        get_proc_address_fn,
                        get_last_error_fn,
                        library_handle,
                        function_name_ptr,
                ...
                library_thunk_ptr = (library_thunk_ptr as usize + core::mem::size_of::<usize>())
                    as *mut IMAGE_THUNK_DATA64;
            }
        }
        base_image_import_table = (base_image_import_table as usize
            + core::mem::size_of::<IMAGE_IMPORT_DESCRIPTOR>() as usize)
            as *mut IMAGE_IMPORT_DESCRIPTOR;
    }
...
pub fn reflective_loader(user_data_ptr_and_dll_bytes: *mut c_void) -> usize {
    ...
    if user_data.function_offset == 0 {
        error_handle("Could not parse the user_data segment")
    }

    // Increment dll_bytes ptr offset to after the user_data ptr.
    let dll_bytes =
        (user_data_ptr_and_dll_bytes as usize + core::mem::size_of::<usize>()) as *mut c_void;

    #[cfg(target_arch = "x86_64")]
    let pe_header = PeFileHeaders64::new(dll_bytes);
    if pe_header.dos_headers.e_magic != PE_MAGIC {
        error_handle("Target DLL does not appear to be a DLL.")
    }
    ...
    let new_dll_base: *mut c_void = match virtual_alloc(
        virtual_alloc_fn,
        get_last_error_fn,
        ptr::null(),
        pe_header.nt_headers.OptionalHeader.SizeOfImage as usize,
    ...
    let image_base_delta =
        new_dll_base as isize - pe_header.nt_headers.OptionalHeader.ImageBase as isize;
    ...
    match process_import_address_tables(
        new_dll_base,
        &pe_header,
        load_library_a_fn,
        get_proc_address_fn,
    ...
    // Execute DllMain
    let entry_point = (new_dll_base as usize
        + pe_header.nt_headers.OptionalHeader.AddressOfEntryPoint as usize)
        as *const FnDllMain;
    let dll_main_func = unsafe { core::mem::transmute::<_, FnDllMain>(entry_point) };
    unsafe {
        dll_main_func(new_dll_base as isize, DLL_PROCESS_ATTACH, 0 as *mut c_void);
    }

    // Call the function the user specified.
    let user_entry_point =
        (new_dll_base as usize + user_data.function_offset as usize) as *const generic_fn;
...
mod tests {
    ...
    use core::time;
    use object::{
        pe::ImageNtHeaders64, read::pe::ImageThunkData, LittleEndian, Object, ObjectSection,
    };
    use std::{
        fs,
        path::{Path, PathBuf},
    ...
    use windows_sys::Win32::{
        ...
        System::{LibraryLoader::LoadLibraryA, Memory::VirtualAlloc},
    };
    ...
    fn test_reflective_loader_get_export_by_hash() -> Result<(), &'static str> {
        ...
        let virtualalloc_addy =
            unsafe { get_export_by_hash(kernel32_base, virtual_alloc_hash).unwrap() };
    ...
    fn test_reflective_loader_simple() -> () {
        ...
        let user_data = UserData {
            function_offset: get_export_address_by_name(read_in_dll_bytes, "demo_init", true)
                .unwrap() as u64,
        };
        ...
        let user_data_ptr_and_dll_bytes =
            user_data_ptr_and_dll_bytes_vec.as_slice().as_ptr() as *mut c_void;
    ...
    fn test_reflective_loader_process_dll_image_relocation() -> anyhow::Result<()> {
        ...
        let pe_header = PeFileHeaders64::new(dll_bytes);
        // Allocate memory for our DLL to be loaded into
        let test_dll_base: *mut c_void = unsafe {
            VirtualAlloc(
                ptr::null(),
                pe_header.nt_headers.OptionalHeader.SizeOfImage as usize,
                MEM_RESERVE | MEM_COMMIT,
        ...
        let image_base_delta =
            test_dll_base as isize - pe_header.nt_headers.OptionalHeader.ImageBase as isize;
        process_dll_image_relocation(test_dll_base, &pe_header, image_base_delta);

        let good_dll_base = unsafe {
            LoadLibraryA(format!("{}\0", test_payload_path.as_path().to_str().unwrap()).as_ptr())
        };
        if good_dll_base == 0 {
        ...
        let good_image_base_delta = good_dll_base
            - pe_file
                .nt_headers()
                .optional_header
                .image_base
                .get(LittleEndian) as isize;

        // Loop over the relocations and check against the updated dll bytes.
        let mut blocks = pe_file
            .data_directories()
            .relocation_blocks(read_in_dll_bytes, &section_table)?
            .unwrap();
        while let Some(block) = blocks.next()? {
            for reloc in block {
                let test_addr =
                    (test_dll_base as usize + reloc.virtual_address as usize) as *mut usize;
                if test_addr as usize
                    > test_dll_base as usize
                        + pe_header.nt_headers.OptionalHeader.SizeOfImage as usize
                {
                    panic!("About to read out of bounds in test")
                }

                let known_good_addr =
                    (good_dll_base as usize + reloc.virtual_address as usize) as *mut usize;
                if known_good_addr as usize
                    > good_dll_base as usize
                        + pe_header.nt_headers.OptionalHeader.SizeOfImage as usize
                {
                    panic!("About to read out of bounds in known good")
                }

                assert_eq!(
                    (unsafe { *test_addr } as usize - image_base_delta as usize),
                    (unsafe { *known_good_addr } as usize - good_image_base_delta as usize)
                );
            }
        }
        Ok(())
    ...
    fn test_reflective_loader_process_import_address_tables() -> anyhow::Result<()> {
        ...
        if let Some(import_table) = pe_file
            .data_directories()
            .import_table(read_in_dll_bytes, &section_table)?
        {
            let mut import_descs = import_table.descriptors()?;
            while let Some(import_desc) = import_descs.next()? {
                let lookup_thunks =
                    import_table.thunks(import_desc.original_first_thunk.get(LittleEndian))?;

                let mut thunks = lookup_thunks.clone();
                while let Some(thunk) = thunks.next::<ImageNtHeaders64>()? {
                    let good_first_few_fn_bytes = unsafe {
                        *((thunk.address() as usize + good_dll_base as usize) as *const usize)
                    };
                    let test_first_few_fn_bytes = unsafe {
                        *((thunk.address() as usize + test_dll_base as usize) as *const usize)
                    };
