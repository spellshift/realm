...
The **assets.read_binary** method returns the asset file contents as bytes.
...
The **file.read_binary** method will read the contents of a file, **returning as bytes**. If the file or directory doesn't exist the method will error to avoid this ensure the file exists, and you have permission to read it.
This function supports globbing with `*` for example:
...
The **random.bytes** method returns randomly generated bytes.
