...
fn get_host_id(host_id_file_path: String) -> Result<String> {
    ...
    if host_id_file.exists() {
        ...
        let _ = host_id_file_obj.write_all(host_id.as_bytes());
    }
    Ok(host_id)
}
...
fn get_primary_ip() -> Result<String> {
    let res = match default_net::get_default_interface() {
        Ok(default_interface) => {
            if !default_interface.ipv4.is_empty() {
...
async fn main_loop(config_path: String, run_once: bool) -> Result<()> {
        ...
        Err(error) => {
            if debug {
                return Err(anyhow::anyhow!(
                    "Unable to get a random session id\n{}",
                    error
                ));
            }
            "DANGER-UNKNOWN".to_string()
    ...
    loop {
        ...
        let time_to_sleep =
            imix_config.callback_config.interval - loop_start_time.elapsed().as_secs();
        if debug {
            println!(
                "[{}]: Sleeping seconds {}",
                (Utc::now().time() - start_time).num_milliseconds(),
        ...
        for exec_future in all_exec_futures.into_iter() {
            ...
            if task_is_finished || !res.is_empty() {
...
pub fn main() -> Result<(), imix::Error> {
    ...
    if let Some(("install", args)) = matches.subcommand() {
...
mod tests {
    ...
    use tavern::{ClaimTasksResponseData, GraphQLResponse, Job, SubmitTaskResult, Tome};
    ...
    fn imix_test_default_ip() {
        ...
        assert!((primary_ip_address != *"DANGER-UNKNOWN"))
    }
...
"#
        ...
        // Define a future for our execution task
        let exec_future = handle_exec_tome(test_tome_input, sender);
...
print("main_loop_test_success")"#
        ...
        let claim_task_response = GraphQLResponse {
            data: Some(ClaimTasksResponseData {
                claim_tasks: vec![test_task.clone(), test_task],
            }),
            errors: None,
            extensions: None,
        };
        ...
        let path_new = String::from(tmp_file_new.path().to_str().unwrap());
        let _ = std::fs::write(
            path_new.clone(),
            format!(
                r#"{{
    ...
    "callback_config": {{
        ...
        runtime.block_on(exec_future).unwrap();
