...
async fn run_benchmark_iteration(
    ...
    let expected_size = 1024 * 1024 * 1024; // 1GB
...
fn bench_portal_throughput(c: &mut Criterion) {
    // 1. Setup Mock HTTP Server (1GB) shared across iterations
    ...
    let mut group = c.benchmark_group("portal");
    group.sample_size(10); // Reduce sample size for long running bench
    group.measurement_time(std::time::Duration::from_secs(120)); // Allow it to run longer

    group.bench_function("download_1gb", |b| {
        b.to_async(&rt).iter(|| async {
            run_benchmark_iteration(addr).await
        })
    });
    group.finish();
}

criterion_group!(benches, bench_portal_throughput);
