...
mod tests {
    ...
    fn test_decrypt() -> Result<()> {
...

        {
            ...
            f.write(&lorem_encrypted)?;
        }
        decrypt_file(path, path_dec.clone(), "TESTINGPASSWORD!".to_string())?;

        let mut hasher = Sha1::new();
        let mut enc_f = File::open(path_dec.clone())?;
    ...
    fn test_decrypt_bad_password() -> Result<()> {
        let tmp_file = NamedTempFile::new()?;
        let path = String::from(tmp_file.path().to_str().unwrap()).clone();
        assert!(decrypt_file(path.clone(), path.clone(), "TESTINGPASSWORD!!".to_string()).is_err());
        assert!(decrypt_file(path.clone(), path.clone(), "TESTINGPASSWORD".to_string()).is_err());
        Ok(())
    }
    ...
    fn test_decrypt_no_file() -> Result<()> {
        let tmp_file = NamedTempFile::new()?;
        let path = String::from(tmp_file.path().to_str().unwrap()).clone();
        assert!(decrypt_file("/I/Dont/Exist".to_string(), path.clone(), "TESTINGPASSWORD!".to_string()).is_err());
        assert!(decrypt_file(path.clone(), "/I/Dont/Exist".to_string(), "TESTINGPASSWORD!".to_string()).is_err());
        Ok(())
    }
}
