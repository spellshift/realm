...
pub fn moveto(old: String, new: String) -> Result<()> {
    ...
    if Path::new(&new).is_dir() {
        fs::remove_dir_all(new.clone())?;
    } else if Path::new(&new).is_file() {
...
mod tests {
    ...
    fn test_moveto_dir() -> anyhow::Result<()> {
        let tmp_dir = tempdir()?;
        let path_old = String::from(tmp_dir.path().to_str().unwrap());
    ...
    fn test_moveto_file() -> anyhow::Result<()> {
        ...
        let path_old = String::from(tmp_file.path().to_str().unwrap());
    ...
    fn test_moveto_dir_overwrites() -> anyhow::Result<()> {
        ...
        let path_old = String::from(tmp_dir_path.to_str().unwrap());
        let path_old_file = String::from(tmp_dir_path.join("myfile2").to_str().unwrap());
        let _ = File::create(path_old_file.clone())?;
        let _ = fs::write(path_old_file, "Hello");
