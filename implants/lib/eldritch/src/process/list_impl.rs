...
pub fn list(starlark_heap: &Heap) -> Result<Vec<Dict>> {
    ...
    for (pid, process) in sys.processes() {
        ...
        tmp_res.insert_hashed(
            const_frozen_string!("pid").to_value().get_hashed().unwrap(),
            Value::new_int(pid.as_u32().try_into().unwrap_or(-1)),
        );
        tmp_res.insert_hashed(
            const_frozen_string!("ppid")
                .to_value()
                .get_hashed()
                .unwrap(),
            Value::new_int(tmp_ppid.try_into().unwrap_or(-1)),
        );
        tmp_res.insert_hashed(
            const_frozen_string!("status")
                .to_value()
                .get_hashed()
                .unwrap(),
            starlark_heap
                .alloc_str(&process.status().to_string())
                .to_value(),
        );
        tmp_res.insert_hashed(
            const_frozen_string!("username")
                .to_value()
                .get_hashed()
                .unwrap(),
            starlark_heap.alloc_str(&tmp_username).to_value(),
        );
        tmp_res.insert_hashed(
            const_frozen_string!("path")
                .to_value()
                .get_hashed()
                .unwrap(),
            starlark_heap
                .alloc_str(&String::from(process.exe().to_str().unwrap()))
                .to_value(),
        );
        tmp_res.insert_hashed(
            const_frozen_string!("command")
                .to_value()
                .get_hashed()
                .unwrap(),
            starlark_heap
        ...
        tmp_res.insert_hashed(
            ...
            starlark_heap
                .alloc_str(&String::from(process.cwd().to_str().unwrap()))
                .to_value(),
        );
        tmp_res.insert_hashed(
            const_frozen_string!("environ")
                .to_value()
                .get_hashed()
                .unwrap(),
            starlark_heap
                .alloc_str(&process.environ().join(" "))
                .to_value(),
        );
        tmp_res.insert_hashed(
            const_frozen_string!("name")
                ...
                .unwrap(),
            starlark_heap
                .alloc_str(&String::from(process.name()))
                .to_value(),
        );
