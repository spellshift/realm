...
pub fn exec(
    ...
    dict_res.insert_hashed(
        const_frozen_string!("stdout")
            .to_value()
            .get_hashed()
            .unwrap(),
        stdout_value.to_value(),
    );

    let stderr_value = starlark_heap.alloc_str(cmd_res.stderr.as_str());
    dict_res.insert_hashed(
        const_frozen_string!("stderr")
            .to_value()
            .get_hashed()
            .unwrap(),
        stderr_value.to_value(),
    );
...
fn handle_exec(path: String, args: Vec<String>, disown: Option<bool>) -> Result<CommandOutput> {
    ...
    if !should_disown {
        ...
        match unsafe { fork().expect("Failed to fork process") } {
            ...
            ForkResult::Child => match unsafe { fork().expect("Failed to fork process") } {
                ForkResult::Parent { child } => {
                    if child.as_raw() < 0 {
