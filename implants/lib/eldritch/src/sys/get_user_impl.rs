...
pub fn get_user(starlark_heap: &Heap) -> Result<Dict> {
    ...
    if let Some(process) = sys.process(Pid::from(pid)) {
        ...
        insert_dict_kv!(dict_user, starlark_heap, "name", user.name().to_string(), String);
        insert_dict_kv!(dict_user, starlark_heap, "gid", *user.group_id(), u32);
        insert_dict_kv!(dict_user, starlark_heap, "groups", Vec::from(user.groups()), Vec<_>);
        ...
        #[cfg(not(target_os="windows"))]
        {
            ...
            insert_dict_kv!(dict_euser, starlark_heap, "groups", Vec::from(euser.groups()), Vec<_>);
