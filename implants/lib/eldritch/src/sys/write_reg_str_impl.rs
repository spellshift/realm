...
pub fn write_reg_str(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        match _regtype.as_ref() {
            ...
            "REG_BINARY" => {
                let data = RegValue{ vtype: REG_BINARY, bytes: _regvalue.as_bytes().to_vec()};
                nkey.set_raw_value(_regname, &data)?;
            },
            "REG_DWORD" => {
                let parsed_value: u32 = u32::from_str_radix(&_regvalue, 10)?;
            ...
            "REG_MULTI_SZ" => {
                let parsed_value: Vec<&str> = _regvalue.split(',').collect();
                nkey.set_value(_regname, &parsed_value)?;
            },
            "REG_RESOURCE_LIST" => {
                let data = RegValue{ vtype: REG_RESOURCE_LIST, bytes: _regvalue.as_bytes().to_vec()};
                nkey.set_raw_value(_regname, &data)?;
            },
            "REG_FULL_RESOURCE_DESCRIPTOR" => {
                let data = RegValue{ vtype: REG_FULL_RESOURCE_DESCRIPTOR, bytes: _regvalue.as_bytes().to_vec()};
                nkey.set_raw_value(_regname, &data)?;
            },
            "REG_RESOURCE_REQUIREMENTS_LIST" => {
                let data = RegValue{ vtype: REG_RESOURCE_REQUIREMENTS_LIST, bytes: _regvalue.as_bytes().to_vec()};
                nkey.set_raw_value(_regname, &data)?;
            },
            "REG_QWORD" => {
                let parsed_value: u64 = u64::from_str_radix(&_regvalue, 10)?;
