    ...
    fn download(&self, url: String, path: String) -> Result<(), String> {
        ...
        runtime.block_on(async {
            ...
            while let Some(chunk_result) = stream.next().await {
                 ...
                 dest.write_all(&chunk).map_err(|e| format!("Error writing to file: {}", e))?;
            }

            dest.flush().map_err(|e| format!("Error flushing file: {}", e))?;
            Ok(())
        })
    }
    ...
    fn get(&self, url: String, headers: Option<BTreeMap<String, String>>) -> Result<BTreeMap<String, Value>, String> {
        ...
        if let Some(h) = headers {
            ...
            for (k, v) in h {
                ...
                let value = HeaderValue::from_bytes(v.as_bytes()).map_err(|e| format!("Invalid header value: {}", e))?;
        ...
        map.insert("status_code".into(), Value::Int(resp.status().as_u16() as i64));
...
mod tests {
    ...
    fn test_get() {
        ...
        server.expect(
            Expectation::matching(request::method_path("GET", "/foo"))
                .respond_with(status_code(200).body("test body").append_header("X-Test", "Value")),
        );
        ...
        if let Value::Dictionary(d) = res.get("headers").unwrap() {
             let dict = d.borrow();
             assert_eq!(dict.get("x-test").or(dict.get("X-Test")).unwrap(), &Value::String("Value".into()));
        } else {
