use crate::{std::StdPivotLibrary, PivotLibrary, ReplHandler};
...
impl MockTransport { fn new() -> Self { Self { calls: Arc::new(Mutex::new(Vec::new())) } } }
impl SyncTransport for MockTransport {
    ...
    fn report_task_output(&self, _r: c2::ReportTaskOutputRequest) -> anyhow::Result<c2::ReportTaskOutputResponse> { Ok(c2::ReportTaskOutputResponse {}) }
    fn reverse_shell(&self, _rx: Receiver<c2::ReverseShellRequest>, _tx: Sender<c2::ReverseShellResponse>) -> anyhow::Result<()> {
...
fn test_reverse_shell_repl_delegation() {
    let transport = Arc::new(MockTransport::new());
    let repl_handler = Arc::new(MockReplHandler { calls: Arc::new(Mutex::new(Vec::new())) });
    let task_id = 123;
