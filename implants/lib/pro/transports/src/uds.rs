...
// UDS is a local trusted channel; use plain protobuf codec (no encryption).
...
/// Cached UDS listener + gRPC channel, keyed by socket path.
/// ImixAgent calls UdsTransport::new() every beacon cycle; caching here prevents
/// re-binding the socket and re-creating the HTTP/2 connection each time.
static UDS_CACHE: std::sync::OnceLock<(
    String,                                         // UDS path
    Arc<tokio::sync::Mutex<UnixListenerStream>>,    // listener stream
...
impl Transport for UdsTransport {
    ...
    fn new(config: Config) -> Result<Self> {
        ...
        log::info!("[uds-client] new() with uri: {}", uri);

        // Expect URI to look like unix:///tmp/agent.sock
        let path = if uri.starts_with("unix://") {
            uri.strip_prefix("unix://").unwrap().to_string()
        } else {
            return Err(anyhow!(
                "Invalid scheme for UDS transport (expected unix://): {}",
                uri
            ));
        };

        log::info!("[uds-client] UDS path: {}", path);

        // If we already have a cached channel for this path, reuse it.
        if let Some((cached_path, _stream, cached_grpc)) = UDS_CACHE.get() {
            if cached_path == &path {
                log::debug!("[uds-client] reusing cached gRPC channel for {}", path);
                return Ok(Self {
                    grpc: Some(cached_grpc.clone()),
                });
            } else {
                return Err(anyhow!(
                    "UdsTransport path changed from {} to {} — restart required",
                    cached_path,
                    path
                ));
            }
        ...
        // First call: bind the listener and create the channel.
        let path_obj = PathBuf::from(&path);
        if path_obj.exists() {
            log::info!("[uds-client] removing stale socket: {}", path_obj.display());
            std::fs::remove_file(&path_obj)?;
        }
        log::info!(
            "[uds-client] binding UDS listener at {} — waiting for Agent A to connect",
            path_obj.display()
        );
        let listener = UnixListener::bind(&path_obj)?;
        let stream = Arc::new(tokio::sync::Mutex::new(UnixListenerStream::new(listener)));

        // We use a dummy endpoint since we are connecting over UDS.
        ...
        let channel = endpoint.connect_with_connector_lazy(tower::service_fn(
            move |uri: tonic::transport::Uri| {
                ...
                log::debug!("[uds-client] connector called for uri: {}", uri);
                async move {
                    use tokio_stream::StreamExt;
                    log::debug!("[uds-client] connector: waiting for next UDS connection...");
                    let mut guard = stream_clone.lock().await;
                    if let Some(conn) = guard.next().await {
                        match conn {
                            Ok(c) => {
                                log::info!(
                                    "[uds-client] connector: accepted connection from Agent A"
                                );
                                Ok(hyper_util::rt::TokioIo::new(c))
                            }
                            Err(e) => {
                                log::error!("[uds-client] connector: accept error: {}", e);
                        ...
                        log::error!("[uds-client] connector: UDS listener stream closed");
                        Err(std::io::Error::new(
        ...
        log::info!("[uds-client] gRPC channel created, transport ready");

        // Cache the stream + channel for future cycles.
        let _ = UDS_CACHE.set((path, stream, grpc.clone()));

        Ok(Self { grpc: Some(grpc) })
    }

    async fn claim_tasks(&mut self, request: ClaimTasksRequest) -> Result<ClaimTasksResponse> {
        log::debug!("[uds-client] claim_tasks()");
    ...
    async fn report_output(
        ...
        log::debug!("[uds-client] report_output()");
    ...
    async fn forward_raw(
        ...
        Err(anyhow!("UDS transport raw forwarding not implemented yet"))
    }
...
impl UdsTransport {
    async fn check_ready(&mut self) -> Result<(), tonic::Status> {
        if self.grpc.is_none() {
            log::error!("[uds-client] check_ready: grpc client is None");
            return Err(tonic::Status::new(
                tonic::Code::FailedPrecondition,
                "grpc client not created".to_string(),
            ));
        }
        log::debug!("[uds-client] check_ready: waiting for gRPC channel to be ready...");
        self.grpc.as_mut().unwrap().ready().await.map_err(|e| {
            log::error!("[uds-client] check_ready: channel not ready: {}", e);
            tonic::Status::new(
                tonic::Code::Unknown,
                format!("Service was not ready: {}", e),
            )
        })?;
        log::debug!("[uds-client] check_ready: channel ready");
        Ok(())
    }
