...
pub use mutations::claim_tasks::{
    ClaimTasksClaimTasks as Task, ClaimTasksClaimTasksJob as Job,
    ClaimTasksClaimTasksJobBundle as Bundle, ClaimTasksClaimTasksJobTome as Tome,
    ClaimTasksClaimTasksJobTomeFiles as File, ClaimTasksInput,
    ResponseData as ClaimTasksResponseData, SessionHostPlatform as HostPlatform,
};
pub use mutations::submit_task_result::{
    ResponseData as SubmitTaskResultResponseData, SubmitTaskResultInput,
...
#[async_trait]
pub trait Executor {
    async fn exec<Variables: Serialize + Send, GraphQLResponse: DeserializeOwned>(
        &self,
...
impl<E: Executor> Client<E> {
    ...
    pub async fn claim_tasks(&self, input: ClaimTasksInput) -> Result<Vec<Task>> {
        ...
        let resp = self
            .exec::<mutations::claim_tasks::Variables, mutations::claim_tasks::ResponseData>(query)
            .await?;
        Ok(resp.claim_tasks)
    }
    ...
    pub async fn submit_task_result(&self, input: SubmitTaskResultInput) -> Result<()> {
        ...
        let query: QueryBody<mutations::submit_task_result::Variables> =
            mutations::SubmitTaskResult::build_query(vars);
    ...
    async fn exec<Variables: Serialize + Send, ResponseData: DeserializeOwned>(
        &self,
        query: QueryBody<Variables>,
    ) -> Result<ResponseData> {
...
mod tests {
    ...
    impl MockTransport {
        pub fn get_client<T: Serialize>(expected_response: T) -> Client<MockTransport> {
            let expected_data = serde_json::to_string(&expected_response)
