    ...
    fn new(uri: String, proxy_uri: Option<String>, server_pubkey: [u8; 32]) -> Result<Self> {
        match uri {
            ...
            s if s.starts_with("http://") || s.starts_with("https://") => {
                #[cfg(feature = "grpc")]
                return Ok(ActiveTransport::Grpc(grpc::GRPC::new(
                    s,
                    proxy_uri,
                    server_pubkey,
                )?));
            ...
            s if s.starts_with("grpc://") || s.starts_with("grpcs://") => {
                #[cfg(feature = "grpc")]
                {
                    ...
                    Ok(ActiveTransport::Grpc(grpc::GRPC::new(
                        new,
                        proxy_uri,
                        server_pubkey,
                    )?))
            ...
            s if s.starts_with("http1://") || s.starts_with("https1://") => {
                #[cfg(feature = "http1")]
                {
                    ...
                    Ok(ActiveTransport::Http(http::HTTP::new(
                        new,
                        proxy_uri,
                        server_pubkey,
                    )?))
            ...
            s if s.starts_with("dns://") => {
                #[cfg(feature = "dns")]
                {
                    Ok(ActiveTransport::Dns(dns::DNS::new(
                        s,
                        proxy_uri,
                        server_pubkey,
                    )?))
