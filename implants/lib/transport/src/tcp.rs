...
#[derive(Debug, Clone)]
pub struct TCP {
    tcp: Option<tonic::client::Grpc<tonic::transport::Channel>>,
}
...
impl Transport for TCP {
    ...
    fn new(callback: String, proxy_uri: Option<String>) -> Result<Self> {
        ...
        let tcp = tonic::client::Grpc::new(channel);
        Ok(Self { tcp: Some(tcp) })
    }
    ...
    pub async fn claim_tasks_impl(
        ...
        if self.tcp.is_none() {
            return Err(tonic::Status::new(
                ...
                "tcp client not created".to_string(),
            ));
        }
        self.tcp.as_mut().unwrap().ready().await.map_err(|e| {
            tonic::Status::new(
                tonic::Code::Unknown,
        ...
        self.tcp.as_mut().unwrap().unary(req, path, codec).await
    }
