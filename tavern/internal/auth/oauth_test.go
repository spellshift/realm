...
func TestNewOAuthLoginHandler(t *testing.T) {
	...
	assert.True(t, claims.IssuedAt.Time.After(time.Now().Add(-60*time.Second)) || claims.IssuedAt.Time.Equal(time.Now().Add(-60*time.Second)))
	assert.True(t, claims.ExpiresAt.Time.After(time.Now().Add(9*time.Minute)))
	assert.True(t, claims.ExpiresAt.Time.Before(time.Now().Add(11*time.Minute)))
}
