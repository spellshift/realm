...
func (c *builderClient) DownloadTome(ctx context.Context, in *DownloadTomeRequest, opts ...grpc.CallOption) (Builder_DownloadTomeClient, error) {
	stream, err := c.cc.NewStream(ctx, &_Builder_serviceDesc.Streams[2], "/builder.Builder/DownloadTome", opts...)
	if err != nil {
		return nil, err
	}
	x := &builderDownloadTomeClient{stream}
...
func _Builder_DownloadTome_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(BuilderServer).DownloadTome(m, &builderDownloadTomeServer{stream})
}
