...
const (
	...
	C2_ReportFact_FullMethodName        = "/c2.C2/ReportFact"
...
type C2Client interface {
	...
	ReportFact(ctx context.Context, in *ReportFactRequest, opts ...grpc.CallOption) (*ReportFactResponse, error)
...
func (c *c2Client) ReportFact(ctx context.Context, in *ReportFactRequest, opts ...grpc.CallOption) (*ReportFactResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ReportFactResponse)
	err := c.cc.Invoke(ctx, C2_ReportFact_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (c *c2Client) CreatePortal(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[CreatePortalRequest, CreatePortalResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
...
type C2Server interface {
	...
	// Report a fact from the host to the server.
	ReportFact(context.Context, *ReportFactRequest) (*ReportFactResponse, error)
	// Report a file from the host to the server.
	// Providing content of the file is optional. If content is provided:
	//   - Hash will automatically be calculated and the provided hash will be ignored.
	//   - Size will automatically be calculated and the provided size will be ignored.
...
func (UnimplementedC2Server) ReportFact(context.Context, *ReportFactRequest) (*ReportFactResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method ReportFact not implemented")
}
...
func _C2_ReportFact_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(ReportFactRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(C2Server).ReportFact(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: C2_ReportFact_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(C2Server).ReportFact(ctx, req.(*ReportFactRequest))
	}
	return interceptor(ctx, in, info, handler)
}
...
func _C2_CreatePortal_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(C2Server).CreatePortal(&grpc.GenericServerStream[CreatePortalRequest, CreatePortalResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type C2_CreatePortalServer = grpc.BidiStreamingServer[CreatePortalRequest, CreatePortalResponse]
...
var C2_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "ReportFact",
			Handler:    _C2_ReportFact_Handler,
		},
		{
