	...
	for i := range columns {
		...
		case builder.FieldSupportedTargets:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field supported_targets", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &b.SupportedTargets); err != nil {
					return fmt.Errorf("unmarshal field supported_targets: %w", err)
				}
			}
		case builder.FieldUpstream:
