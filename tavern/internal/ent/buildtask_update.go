...
func (btu *BuildTaskUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if btu.mutation.BuilderProfileCleared() {
	...
	if nodes := btu.mutation.BuilderProfileIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: false,
			Table:   buildtask.BuilderProfileTable,
...
func (btuo *BuildTaskUpdateOne) sqlSave(ctx context.Context) (_node *BuildTask, err error) {
	...
	if btuo.mutation.BuilderProfileCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: false,
			Table:   buildtask.BuilderProfileTable,
	...
	if nodes := btuo.mutation.BuilderProfileIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: false,
			Table:   buildtask.BuilderProfileTable,
