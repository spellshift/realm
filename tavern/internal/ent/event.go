...
// was not loaded in eager-loading, or loaded but was not found.
func (e EventEdges) ShellOrErr() (*Shell, error) {
	if e.Shell != nil {
		return e.Shell, nil
...
func (*Event) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case event.ForeignKeys[2]: // quest_events
			values[i] = new(sql.NullInt64)
		case event.ForeignKeys[3]: // shell_events
			values[i] = new(sql.NullInt64)
		case event.ForeignKeys[4]: // user_events
			values[i] = new(sql.NullInt64)
		default:
...
func (e *Event) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case event.ForeignKeys[3]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field shell_events", value)
			} else if value.Valid {
				e.shell_events = new(int)
				*e.shell_events = int(value.Int64)
			}
...
// QueryShell queries the "shell" edge of the Event entity.
func (e *Event) QueryShell() *ShellQuery {
	return NewEventClient(e.config).QueryShell(e)
}
