...
func (_q *FileQuery) QueryTomes() *TomeQuery {
...
func (_q *FileQuery) WithTomes(opts ...func(*TomeQuery)) *FileQuery {
	query := (&TomeClient{config: _q.config}).Query()
...
func (_q *FileQuery) loadTomes(ctx context.Context, query *TomeQuery, nodes []*File, init func(*File), assign func(*File, *Tome)) error {
...
func (_q *FileQuery) WithNamedTomes(name string, opts ...func(*TomeQuery)) *FileQuery {
	query := (&TomeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
