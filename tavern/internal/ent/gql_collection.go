	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "scheduledtasks":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&ScheduledTaskClient{config: h.config}).Query()
			)
			args := newScheduledTaskPaginateArgs(fieldArgs(ctx, new(ScheduledTaskWhereInput), path...))
			if err := validateFirstLast(args.first, args.last); err != nil {
				return fmt.Errorf("validate first and last in path %q: %w", path, err)
			}
			pager, err := newScheduledTaskPager(args.opts, args.last != nil)
			if err != nil {
				return fmt.Errorf("create new pager in path %q: %w", path, err)
			}
			if query, err = pager.applyFilter(query); err != nil {
				...
				if hasPagination || ignoredEdges {
					...
					h.loadTotal = append(h.loadTotal, func(ctx context.Context, nodes []*Host) error {
						...
						query.Where(func(s *sql.Selector) {
							...
							s.Where(sql.InValues(joinT.C(host.ScheduledTasksPrimaryKey[1]), ids...))
							s.Select(joinT.C(host.ScheduledTasksPrimaryKey[1]), sql.Count("*"))
							s.GroupBy(joinT.C(host.ScheduledTasksPrimaryKey[1]))
						})
						if err := query.Select().Scan(ctx, &v); err != nil {
							return err
						}
					...
					h.loadTotal = append(h.loadTotal, func(_ context.Context, nodes []*Host) error {
						for i := range nodes {
							n := len(nodes[i].Edges.ScheduledTasks)
							if nodes[i].Edges.totalCount[8] == nil {
								nodes[i].Edges.totalCount[8] = make(map[string]int)
							}
							nodes[i].Edges.totalCount[8][alias] = n
			...
			path = append(path, edgesField, nodeField)
			if field := collectedField(ctx, path...); field != nil {
				if err := query.collectField(ctx, false, opCtx, *field, path, mayAddCondition(satisfies, scheduledtaskImplementors)...); err != nil {
					return err
				}
			}
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := entgql.LimitPerRow(host.ScheduledTasksPrimaryKey[1], limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
			}
			h.WithNamedScheduledTasks(alias, func(wq *ScheduledTaskQuery) {
				*wq = *query
			})
		case "createdAt":
		...
		case "scheduledHosts":
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					...
					st.loadTotal = append(st.loadTotal, func(ctx context.Context, nodes []*ScheduledTask) error {
						...
						var v []struct {
							NodeID int `sql:"scheduled_task_id"`
							Count  int `sql:"count"`
						}
						query.Where(func(s *sql.Selector) {
							joinT := sql.Table(scheduledtask.ScheduledHostsTable)
							s.Join(joinT).On(s.C(host.FieldID), joinT.C(scheduledtask.ScheduledHostsPrimaryKey[1]))
							s.Where(sql.InValues(joinT.C(scheduledtask.ScheduledHostsPrimaryKey[0]), ids...))
							s.Select(joinT.C(scheduledtask.ScheduledHostsPrimaryKey[0]), sql.Count("*"))
							s.GroupBy(joinT.C(scheduledtask.ScheduledHostsPrimaryKey[0]))
						})
						if err := query.Select().Scan(ctx, &v); err != nil {
							return err
						}
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := entgql.LimitPerRow(scheduledtask.ScheduledHostsPrimaryKey[0], limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
