	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "shell":
			var (
				alias = field.Alias
				path  = append(path, alias)
				query = (&ShellClient{config: e.config}).Query()
			)
		...
		case "events":
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					...
					s.loadTotal = append(s.loadTotal, func(ctx context.Context, nodes []*Shell) error {
						...
						var v []struct {
							NodeID int `sql:"shell_events"`
							Count  int `sql:"count"`
						}
						query.Where(func(s *sql.Selector) {
							s.Where(sql.InValues(s.C(shell.EventsColumn), ids...))
						})
						if err := query.GroupBy(shell.EventsColumn).Aggregate(Count()).Scan(ctx, &v); err != nil {
							return err
						}
						m := make(map[int]int, len(v))
						for i := range v {
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				if oneNode {
					...
					modify := entgql.LimitPerRow(shell.EventsColumn, limit, pager.orderExpr(query))
					query.modifiers = append(query.modifiers, modify)
				}
			} else {
				query = pager.applyOrder(query)
