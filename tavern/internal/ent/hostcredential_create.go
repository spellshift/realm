...
func (hcc *HostCredentialCreate) SetShellID(id int) *HostCredentialCreate {
	hcc.mutation.SetShellID(id)
	return hcc
}

// SetNillableShellID sets the "shell" edge to the Shell entity by ID if the given value is not nil.
func (hcc *HostCredentialCreate) SetNillableShellID(id *int) *HostCredentialCreate {
	if id != nil {
		hcc = hcc.SetShellID(*id)
	}
	return hcc
}

// SetShell sets the "shell" edge to the Shell entity.
func (hcc *HostCredentialCreate) SetShell(s *Shell) *HostCredentialCreate {
	return hcc.SetShellID(s.ID)
}
...
func (hcc *HostCredentialCreate) createSpec() (*HostCredential, *sqlgraph.CreateSpec) {
	...
	if nodes := hcc.mutation.ShellIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
		...
		_node.shell_reported_credentials = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
