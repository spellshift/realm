...
func (hpq *HostProcessQuery) QueryShell() *ShellQuery {
	query := (&ShellClient{config: hpq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := hpq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (hpq *HostProcessQuery) WithShell(opts ...func(*ShellQuery)) *HostProcessQuery {
	query := (&ShellClient{config: hpq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (hpq *HostProcessQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*HostProcess, error) {
	...
	if hpq.withHost != nil || hpq.withTask != nil || hpq.withShell != nil || hpq.withShellTask != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := hpq.withShell; query != nil {
		if err := hpq.loadShell(ctx, query, nodes, nil,
			func(n *HostProcess, e *Shell) { n.Edges.Shell = e }); err != nil {
			return nil, err
		}
	}
...
func (hpq *HostProcessQuery) loadShell(ctx context.Context, query *ShellQuery, nodes []*HostProcess, init func(*HostProcess), assign func(*HostProcess, *Shell)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*HostProcess)
	for i := range nodes {
		if nodes[i].shell_reported_processes == nil {
			continue
		}
		fk := *nodes[i].shell_reported_processes
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "shell_reported_processes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
