...
func (*Screenshot) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case screenshot.FieldPath, screenshot.FieldOwner, screenshot.FieldGroup, screenshot.FieldPermissions, screenshot.FieldHash:
	...
	for i := range columns {
		...
		case screenshot.ForeignKeys[3]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				s.task_reported_screenshots = new(int)
				*s.task_reported_screenshots = int(value.Int64)
			}
		default:
...
func (s *Screenshot) String() string {
	...
	builder.WriteString(s.Permissions)
	builder.WriteString(", ")
