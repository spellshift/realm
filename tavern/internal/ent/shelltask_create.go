...
func (stc *ShellTaskCreate) AddScreenshotIDs(ids ...int) *ShellTaskCreate {
	stc.mutation.AddScreenshotIDs(ids...)
	return stc
}

// AddScreenshots adds the "screenshots" edges to the Screenshot entity.
func (stc *ShellTaskCreate) AddScreenshots(s ...*Screenshot) *ShellTaskCreate {
	ids := make([]int, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return stc.AddScreenshotIDs(ids...)
}
...
func (stc *ShellTaskCreate) createSpec() (*ShellTask, *sqlgraph.CreateSpec) {
	...
	if nodes := stc.mutation.ScreenshotsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
