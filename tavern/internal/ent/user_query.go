...
func (_q *UserQuery) QueryTomes() *TomeQuery {
...
func (_q *UserQuery) QueryActiveShells() *ShellQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withActiveShells: _q.withActiveShells.Clone(),
...
func (_q *UserQuery) WithTomes(opts ...func(*TomeQuery)) *UserQuery {
	query := (&TomeClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithActiveShells(opts ...func(*ShellQuery)) *UserQuery {
	query := (&ShellClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withActiveShells = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withActiveShells; query != nil {
		if err := _q.loadActiveShells(ctx, query, nodes,
	...
	for name, query := range _q.withNamedActiveShells {
		if err := _q.loadActiveShells(ctx, query, nodes,
...
func (_q *UserQuery) loadTomes(ctx context.Context, query *TomeQuery, nodes []*User, init func(*User), assign func(*User, *Tome)) error {
...
func (_q *UserQuery) loadActiveShells(ctx context.Context, query *ShellQuery, nodes []*User, init func(*User), assign func(*User, *Shell)) error {
...
func (_q *UserQuery) WithNamedTomes(name string, opts ...func(*TomeQuery)) *UserQuery {
	query := (&TomeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithNamedActiveShells(name string, opts ...func(*ShellQuery)) *UserQuery {
	query := (&ShellClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedActiveShells == nil {
		_q.withNamedActiveShells = make(map[string]*ShellQuery)
	}
	_q.withNamedActiveShells[name] = query
	return _q
}
