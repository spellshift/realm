...
// A WebsocketTaskInputMessage is used to deliver output from a shell running on an agent.
...
// NewWebsocketTaskOutputMessage creates a new output message to be sent to the browser on the websocket.
func NewWebsocketTaskOutputMessage(task *ent.ShellTask) *WebsocketTaskOutputMessage {
	var creatorID int
	if task.Edges.Creator != nil {
		creatorID = task.Edges.Creator.ID
	}

	return &WebsocketTaskOutputMessage{
...
func NewWebsocketPortalUpgradeMessage(portal *ent.Portal) *WebsocketControlFlowMessage {
...
// NewWebsocketPortalDowngradeMessage indicates that the connection has been closed.
func NewWebsocketPortalDowngradeMessage(portal *ent.Portal) *WebsocketControlFlowMessage {
	return &WebsocketControlFlowMessage{
		Kind:     WebsocketMessageKindControlFlow,
		Signal:   WebsocketControlFlowSignalPortalDowngrade,
		PortalID: portal.ID,
	}
}
