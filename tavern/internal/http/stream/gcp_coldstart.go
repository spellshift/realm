...
// PreventGCPPubSubColdStarts by publishing noop messages on an interval.
...
func PreventPubSubColdStarts(ctx context.Context, interval time.Duration, topicShellOutput string, topicShellInput string) {
	if interval == 0 {
		slog.WarnContext(ctx, "gcppubsub coldstart polling disabled due to 0ms interval, this may impact shell latency")
		return
	}
	if interval < 1*time.Millisecond {
		slog.WarnContext(ctx, "gcppubsub coldstart polling interval less than minimum, setting to 1 millisecond")
	...
	for {
		...
		case <-ticker.C:
			if err := pubOutput.Send(ctx, &pubsub.Message{
				...
				slog.ErrorContext(ctx, "failed to publish shell output keep-alive no op. GCP coldstart latency may be encountered.")
			}
			if err := pubInput.Send(ctx, &pubsub.Message{
				...
				slog.ErrorContext(ctx, "failed to publish shell input keep-alive no op. GCP coldstart latency may be encountered.")
			}
		}
	}
}
