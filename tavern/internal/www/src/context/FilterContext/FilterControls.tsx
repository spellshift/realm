...
function getFilterFields(pathname: string): FilterFieldType[] | null {
    ...
    if (pathname === '/assets') {
        return [FilterFieldType.ASSET_CREATOR];
    }
...
export default function FilterControls() {
    ...
    const renderFilterComponent = (field: FilterFieldType) => {
        ...
        else if (field === FilterFieldType.ASSET_CREATOR) {
            return (
                <div key={field}>
                    <FreeTextSearch
                        key={field}
                        isDisabled={filters.isLocked}
                        defaultValue={filters.assetCreator}
                        setSearch={(newValue) => updateFilters({ 'assetCreator': newValue })}
