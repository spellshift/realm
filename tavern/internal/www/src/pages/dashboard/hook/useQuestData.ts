...
import { TagEdge, TaskQueryTopLevel } from "../../../utils/interfacesQuery";
...
type QuestFormattedData = {
    tomeUsage: Array<any>;
    taskTimeline: Array<any>;
    taskTactics: Array<string>;
    groupUsage: Array<any>;
    serviceUsage: Array<any>;
    totalQuests: number;
    totalOutput: number;
    totalTasks: number;
    totalErrors: number;
...
export const useQuestData = (data: TaskQueryTopLevel | undefined) => {
    ...
    const formatQuestData = useCallback((tasks: Array<any>) => {
        ...
        for (let index in tasks) {
            const groupTag = tasks[index]?.node?.beacon?.host?.tags?.edges.find((tag: TagEdge) => tag.node.kind === "group");
            const serviceTag = tasks[index]?.node?.beacon?.host?.tags?.edges.find((tag: TagEdge) => tag.node.kind === "service");
            applyUniqueTermCount(tasks[index]?.node?.quest?.id, tasks[index]?.node?.quest?.id, false, uniqueQuestCount);
            applyUniqueTermCount(groupTag?.node?.name, groupTag?.node?.id, tasks[index]?.node?.error && tasks[index]?.node?.error.length > 0, uniqueGroup);
            applyUniqueTermCount(serviceTag?.node?.name, serviceTag?.node?.id, tasks[index]?.node?.error && tasks[index]?.node?.error.length > 0, uniqueService);
            applyUniqueTermCount(tasks[index]?.node?.quest?.tome?.name, tasks[index]?.node?.quest?.tome.id, tasks[index]?.node?.error && tasks[index]?.node?.error.length > 0, uniqueTomeCount);
            modifyTaskTimeline(tasks[index], tasksTimeline);
            modifyUniqueTactics(tasks[index], uniqueTactics);

            if (tasks[index]?.node?.outputSize > 0) tasksWithOutput += 1;
            if (tasks[index]?.node?.error && tasks[index]?.node?.error?.length > 0) tasksWithError += 1;
    ...
    useEffect(() => {
        if (data && data?.tasks?.edges && data?.tasks?.edges.length > 0) {
            setLoading(true);
            formatQuestData(data?.tasks?.edges);
        }
    }, [data, formatQuestData]);
