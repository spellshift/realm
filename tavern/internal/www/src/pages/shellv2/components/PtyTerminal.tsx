...
const PtyTerminal: React.FC<PtyTerminalProps> = ({ portalId, pivotId, shellId, onConnectionStatusChange }) => {
  ...
  useEffect(() => {
    ...
    let wsUrl = `${scheme}://${window.location.host}/portals/pty/ws?portal_id=${portalId}&shell_id=${shellId}`;
    if (pivotId) {
      wsUrl = `${scheme}://${window.location.host}/portals/pty/ws?pivot_id=${pivotId}`;
    }
    ...
    <div className="flex-grow flex flex-col relative rounded border border-[#333] h-full overflow-hidden">
        {connectionStatus !== "connected" && (
            <div className="absolute top-0 w-full z-10 bg-yellow-600/20 text-yellow-500 text-xs px-2 py-1 text-center">
                {connectionStatus === "connecting" ? "Connecting to PTY session..." : `Disconnected ${errorMessage ? "- " + errorMessage : ""}`}
            </div>
        )}
