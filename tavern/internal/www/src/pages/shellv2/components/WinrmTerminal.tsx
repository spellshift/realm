...
const WinrmTerminal: React.FC<WinrmTerminalProps> = ({ portalId, target, pivotId, shellId, isActive, onConnectionStatusChange }) => {
  ...
  useEffect(() => {
    ...
    let wsUrl = `${scheme}://${window.location.host}/portals/winrm/ws?portal_id=${portalId}&target=${encodeURIComponent(target)}&shell_id=${shellId}`;
    if (pivotId) {
      wsUrl = `${scheme}://${window.location.host}/portals/winrm/ws?pivot_id=${pivotId}`;
    }
    ...
    ws.onopen = () => {
      ...
      term.write(`\r\n\x1b[32mConnected to WinRM portal for ${target}\x1b[0m\r\n`);
    };
    ...
    ws.onerror = (e) => {
      console.error("WinRM WebSocket error:", e);
      setConnectionStatus("disconnected");
      onConnectionStatusChange?.("disconnected");
    };
    ...
    <div className="flex-grow flex flex-col relative rounded border border-[#333] h-full overflow-hidden">
        {connectionStatus !== "connected" && (
            <div className="absolute top-0 w-full z-10 bg-yellow-600/20 text-yellow-500 text-xs px-2 py-1 text-center">
                {connectionStatus === "connecting" ? "Connecting to WinRM server..." : `Disconnected ${errorMessage ? "- " + errorMessage : ""}`}
            </div>
        )}
