        ...
        <AccessGate>
            <div className="flex flex-col h-screen p-5 bg-[#1e1e1e] text-[#d4d4d4]">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <h1 className="text-xl font-bold">Eldritch Shell for {data?.node?.beacon?.name} on <a href={`/hosts/${data?.node?.beacon?.host?.id}`} className="text-blue-400 hover:underline">{data?.node?.beacon?.host?.name}</a></h1>
                </div>
